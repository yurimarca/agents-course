...
In this unit, we'll develop our agent using a structured Python project. This approach allows us to organize our code across multiple files, each handling a specific part of the application.  

### Project Structure  
- **`tools.py`** – Contains auxiliary tools for our agent.  
- **`retriever.py`** – Implements retrieval functions to support the agent.  
- **`agent.py`** – Combines all components into a fully functional agent, which we'll finalize in the last part of this unit.  

For a detailed reference on the project structure, check out [this HF Space](#). Feel free to clone it and experiment with it!
...
Let's understand this tool step-by-step. 
- The docstring help the agent understand when and how to use this tool
...
def tools_node(data):
    ...
    if guest_name and guest_name in guest_info_result_cleaned:
        ...
        response = "I'm sorry, I couldn't find that guest."
...
```python
🎩 Alfred's Response:
Based on the information provided, Lady Ada Lovelace is a notable mathematician and is celebrated for her significant contributions to the field of computing. She is particularly recognized as the first computer programmer for her work on Charles Babbage's Analytical Engine. Her relation to you is described as a friend, and her contact information includes the email address ada.lovelace@example.com.
```

What's happening in this final step:
- We initialize a Hugging Face model using the `HuggingFaceHub` class
- We create our agent (Alfred) as a `StateGraph`, that combines 2 nodes (`search_guest`, `generate_response`) using an edge
- We ask Alfred to retrieve information about a guest named "Lady Ada Lovelace"
