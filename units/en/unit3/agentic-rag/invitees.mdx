...
def search_guest_node(data):
    """Retrieves detailed information about gala guests based on their name or relation."""
    query = data['messages'][0].content  # Obtener la consulta directamente
    results = bm25_retriever.invoke(query)
    
    guest_info = "\n\n".join([doc.page_content for doc in results[:1]]) if results else "No matching guest information found."
    return {"guest_info": guest_info}

builder.add_node("search_guest", search_guest_node)

def generate_response(data):
    if data["guest_info"] == "No matching guest information found.":
        return {"messages": "I'm sorry, I couldn't find that guest."}

    prompt = f"{data['messages'][0].content}\n{data['guest_info']}. Use only the info retrieved as source.\n"
    response_text = llm.invoke(prompt).replace(prompt, "").strip()
    
    return {"messages": response_text}

builder.add_node("generate_response", generate_response)

builder.set_entry_point("search_guest")
builder.add_edge("search_guest", "generate_response")
