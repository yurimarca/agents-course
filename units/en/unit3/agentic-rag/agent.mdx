...
# Second interaction (referencing the first avoiding a reset)
...
Notice how three different approaches for agent did not directly coupled the memory with the agents, any reason for that 🧐...
* smolagents: will not preserve memory across different runs.
* Llama-index: needs an additional context construct added to the run.
* LangGraph: can retrieve the previous message or use [MemorySaver](https://langchain-ai.github.io/langgraph/tutorials/introduction/#part-3-adding-memory-to-the-chatbot)
